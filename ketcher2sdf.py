...
from core_2 import ketcher_combination_to_sdf
from core_1 import align_with_zmat, str2bool

def process_and_align_ketcher(input_mdl, align=False, reference_sdf=None, output_base='molecule',
                            ...
                            heavy_atom_only=False, zmat_gen_method=1, match_chiral_tag=True):
    """
    Process Ketcher MDL file to generate 3D SDF files and optionally align them to a reference structure.
    
    Args:
        input_mdl (str): Path to input Ketcher MDL file containing combinatorial ligands
    ...
    try:
        # Generate SDF files from Ketcher MDL
        # Create output directory if not exists
        os.makedirs(os.path.dirname(working_output_base) or '.', exist_ok=True)
        
        # Pass the full directory path to ketcher_combination_to_sdf
        working_dir = os.path.dirname(working_output_base) or '.'
        base_name = os.path.basename(working_output_base)
        
        # First get the files with default naming
        sdf_files = ketcher_combination_to_sdf(input_mdl, working_dir)
        
        # Rename files to match requested output base name
        renamed_files = []
        for i, file in enumerate(sdf_files, 1):
            new_name = os.path.join(working_dir, f"{base_name}-{i}.sdf")
            os.rename(file, new_name)
            renamed_files.append(new_name)
        
        # Convert relative paths to absolute paths
        sdf_files = [os.path.abspath(f) for f in renamed_files]
        
        if not sdf_files:
            raise RuntimeError("No SDF files were generated from the Ketcher MDL file")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Convert Ketcher MDL to 3D SDF files with optional alignment')
    
    # Required arguments
    parser.add_argument('-i', '--input', required=True,
                      help='Input Ketcher MDL file path')
    ...
    # Add new patterns argument to alignment options group
    align_group.add_argument('--patterns', '-p',
                      help='Path to JSON file containing symmetry equivalent patterns')
    
    # Add new heavy-atom-only argument to alignment options group
    ...
    # Add new zmat-gen-method argument to alignment options group
    ...
    # Process Ketcher MDL and align if requested
    output_files = process_and_align_ketcher(
